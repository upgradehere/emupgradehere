...
AS SELECT mp.mcu_program_id,
   ...
   FROM mcu_program_m mp
     ...
     LEFT JOIN mcu_t ON mcu_t.mcu_program_id = mp.mcu_program_id AND mcu_t.company_id = company_m.company_id AND mcu_t.deleted_at IS NULL
  WHERE mp.deleted_at IS NULL AND company_m.deleted_at IS NULL
