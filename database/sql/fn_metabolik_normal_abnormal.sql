-- DROP FUNCTION public.fn_metabolik_normal_abnormal(int4);

CREATE OR REPLACE FUNCTION public.fn_metabolik_normal_abnormal(mcu_program_idx integer)
 RETURNS TABLE(count_abnormal integer, count_normal integer)
 LANGUAGE sql
 STABLE
AS $function$
...
SELECT
    CAST(COUNT(CASE WHEN total_abnormal_examinations >= 3 THEN 1 END) AS INT) AS count_abnormal,
    CAST(COUNT(CASE WHEN total_abnormal_examinations < 3 THEN 1 END) AS INT) AS count_normal
FROM final_summary;
$function$
;
