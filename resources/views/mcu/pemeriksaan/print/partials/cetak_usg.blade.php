@if (isset($usg->is_import) && !$usg->is_import)
    @if (!empty($usg))
...
@if((isset($usg->is_import) && !$usg->is_import) && !empty($usg->image_file))
    <div class="page-break"></div>
@endif

@if(!empty($usg->image_file))
    @php
        $images = collect(json_decode($usg->image_file, true))->sort()->values();
    @endphp
