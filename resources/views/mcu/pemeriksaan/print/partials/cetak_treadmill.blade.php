@if (isset($treadmill->is_import) && !$treadmill->is_import)
    @if (!empty($treadmill))
    ...
    @if(isset($doctor_sign[$treadmill->doctor_id]))
        ...
        <table style="width: 100%; font-size: 13px;" cellpadding="3">
            <tr>
                ...
                <td style="width: 40%; text-align: center; vertical-align: bottom;">
                    <img src="{{ public_path('uploads/doctor_sign/'.$doctor_sign[$treadmill->doctor_id]) }}" style="max-width: 150px;"><br>
                    <b>{{ $doctor_list[$treadmill->doctor_id] ?? '' }} </b>
...
@if((isset($treadmill->is_import) && !$treadmill->is_import) && !empty($treadmill->image_file))
    <div class="page-break"></div>
@endif

@if(!empty($treadmill->image_file))
    @php
        $images = collect(json_decode($treadmill->image_file, true))->sort()->values();
    @endphp
    @foreach ($images as $key => $image_file)
        <div style="text-align: center; padding-top: 20px;">
            <img src="{{ public_path('uploads/treadmill/'.$image_file) }}" style="max-width: 710px; max-height: 600px;">
        </div>
        @if(count(json_decode($treadmill->image_file,true)) == $key)
                <div class="page-break"></div>
        @endif
    @endforeach
@endif
