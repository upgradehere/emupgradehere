@if (isset($rontgen->is_import) && !$rontgen->is_import)
	...
	@if(isset($doctor_sign[$rontgen->doctor_id]))
	    ...
	    <table style="width: 100%; font-size: 13px;" cellpadding="3">
	         <tr>
	            ...
	            <td style="width: 40%; text-align: center; vertical-align: bottom;">
	                <img src="{{ public_path('uploads/doctor_sign/'.$doctor_sign[$rontgen->doctor_id]) }}" style="max-width: 150px;"><br>
	                <b>{{ $doctor_list[$rontgen->doctor_id] ?? '' }} </b>
...
@if(!empty($rontgen->image_file))
    @php
        $images = collect(json_decode($rontgen->image_file, true))->sort()->values();
    @endphp
