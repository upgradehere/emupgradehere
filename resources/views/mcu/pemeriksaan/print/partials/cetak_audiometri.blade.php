...
@if (isset($audiometri->is_import) && !$audiometri->is_import)
    ...
    @if(isset($doctor_sign[$audiometri->doctor_id]))
        ...
        <table style="width: 100%; font-size: 13px;" cellpadding="3">
             <tr>
                ...
                <td style="width: 40%; text-align: center; vertical-align: bottom;">
                    <img src="{{ public_path('uploads/doctor_sign/'.$doctor_sign[$audiometri->doctor_id]) }}" style="max-width: 150px;"><br>
                    <b>{{ $doctor_list[$audiometri->doctor_id] ?? '' }} </b>
...
@if(!empty($audiometri->image_file))
    @foreach (json_decode($audiometri->image_file,true) as $key => $image_file)
        ...
        @if(count(json_decode($audiometri->image_file,true)) == $key)
                <div class="page-break"></div>
        @endif
    @endforeach
@endif